...
export const getStaticProps: GetStaticProps = async function(a) {
    return {props: {allPosts: getPosts().map(x => ({...x, tags: x.tags.join(" ")}))}}
}
...
export default function Search(props: {allPosts: Post[]}) {
    ...
    if(index != null && typeof router.query.q == 'string') {
        ...
        let R: any = results.map(x => {
            ...
            return <li><a href={`/blog/${post?.slug}`} className="text-blue-500">{post?.title} - <small>{post?.date}</small></a></li>
        })
        if(R.length === 0) {
        ...
        return <div className="container mx-auto text-white px-10 py-10 mt-10">
            <h1 className="text-lg font-bold">Search results for "{router.query.q}": </h1>
            {R}
  ...
  <div className="p-2 bg-indigo-800 items-center text-indigo-100 leading-none lg:rounded-full flex lg:inline-flex" role="alert">
    ...
    <span className="font-regular mr-2 text-left flex-auto">I haven't spent enough time on the search functionality and it might suck.</span>
  </div>
</div>
            </div>
    }
